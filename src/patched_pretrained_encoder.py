...
def train_model(model, train_loader, test_loader, conf):
    ...
    encoder_freeze_epochs = conf['train'].get('encoder_freeze_epochs', 20)  # Default: freeze for 10 epochs

    # Identify encoder parameters
    encoder_params = []
    for name, param in model.named_parameters():
        if "encoder" in name:  # assuming encoder is named like 'encoder.xxx'
            encoder_params.append(param)
            param.requires_grad = False  # Freeze

    logging.info(f"Encoder frozen for the first {encoder_freeze_epochs} epochs.")
    ...
    for epoch in tqdm(range(1, conf['train']['num_epochs'] + 1), desc="Training Epochs", leave=False, unit='epoch'):
        if epoch == encoder_freeze_epochs + 1:
            logging.info(f"Unfreezing encoder at epoch {epoch}")
            for param in encoder_params:
                param.requires_grad = True

            # Reinitialize optimizer to include encoder parameters
            optimizer = torch.optim.Adam(
                model.parameters(),
                lr=conf['train']['learning_rate'],
            )
...
def test_model(model, test_loader, conf, return_metrics_only=False):
    model.eval()
    dice_scores = []
    iou_scores = []
    val_loss = 0.0
    ...
    with torch.no_grad():
        for idx, batch in enumerate(test_loader):
            ...
            loss = criterion(preds, mask_patches)
            val_loss += loss.item()

            # Reconstruct predicted and true full-size masks
            ...
            if not return_metrics_only:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(viz_dir, f'test_image_{idx}.png'))
                plt.close()
