...
def to_float(val):
    ...
    return float(val)  # already a float
...
def compute_metrics(preds, targets, threshold=0.5, alpha=0.5, beta=0.5):
    ...
    metrics = {}
    metrics['Dice']       = to_float(compute_dice(preds_detached, targets_detached, threshold))
    metrics['softDice']   = to_float(compute_soft_dice(preds_detached, targets_detached))
    metrics['clDice']     = to_float(compute_cldice(preds_detached.squeeze(), targets_detached.squeeze(), threshold))
    metrics['softclDice'] = to_float(soft_cldice()(targets_detached, preds_detached))
    metrics['IoU']        = to_float(compute_iou(preds_detached, targets_detached, threshold))
    metrics['Tversky']    = to_float(compute_tversky(preds_detached, targets_detached, alpha, beta, threshold))
    metrics['Hausdorff']  = to_float(compute_hausdorff(preds_detached, targets_detached, threshold))
    metrics['HD95']       = to_float(compute_hd95(preds_detached, targets_detached, threshold))
    return metrics
    ...
    def fn(preds, targets, dist=None):
        ...
        if use_topo and name in ('dice','tversky','focal_tversky','bce'):
            if name=='focal_tversky' and combine=='dice' and w_comb > 0:
                return combined_topo_tversky_dice(probs, targets, dist,
                    topo_alpha=topo_a, tv_alpha=tversky_a, tv_beta=tversky_b, tv_gamma=focal_tversky_gamma,
                    dice_weight=w_comb, beta=topo_b, eps=eps)
            if name=='bce' and combine=='dice' and w_comb > 0:
                return bce_dice_topographic_loss(probs, targets, dist,
                    alpha=topo_a, dice_weight=w_comb, beta=topo_b, eps=eps)
            return topographic_loss(probs, targets, dist, base_loss=name, alpha=topo_a, beta=topo_b, eps=eps)
        else:
            loss_map = {
                ...
                'focal_tversky': lambda p,t: focal_tversky_loss(p, t, alpha=focal_tversky_gamma, beta=tversky_b, gamma=focal_tversky_gamma),
