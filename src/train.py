...
def test_model(model, test_loader, conf, save_dir=None, return_metrics_only=False):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(test_loader):
            ...
            dice = compute_dice(pred_full, mask_full)
            iou = compute_iou(pred_full, mask_full)
            ...
            if not return_metrics_only:
                ...
                img_path = os.path.join(test_loader.dataset.root_dir, "enhanced_images", batch['filename'][0])
                ...
                plt.tight_layout()
                save_path = os.path.join(save_dir, f'test_image_{idx}.png')
    ...
    return val_loss / len(test_loader), np.mean(dice_scores), np.mean(iou_scores)
...
def train_model(model, train_loader, test_loader, conf):
    ...
    best_dice, best_val_loss = 0.0, float('inf')
    ...
    for epoch in range(1, conf['train']['num_epochs'] + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            if use_topo:
                images, masks, distances = [x.cuda() for x in batch]
                loss = loss_fn(model(images), masks, distances) / accumulation_steps
            else:
                images, masks = [x.cuda() for x in batch]
        ...
        avg_val_loss, avg_dice, avg_iou = test_model(model, test_loader, conf, return_metrics_only=False, save_dir=save_dir)
        ...
        if epoch % conf['train']['log_interval'] == 0 or epoch == conf['train']['num_epochs']:
            logging.info(
                ...
                f"IoU: {avg_iou:.4f} | LR: {optimizer.param_groups[0]['lr']:.6f}"
            )
...
if __name__ == '__main__':
    ...
    log_dir = os.path.join('logs', conf['model']['name'])
    os.makedirs(log_dir, exist_ok=True)
    log_filename = os.path.join(log_dir, f"{conf['train']['experiment_name']}_{conf['train']['timestamp']}.log")
    logging.basicConfig(
        level=logging.INFO,
