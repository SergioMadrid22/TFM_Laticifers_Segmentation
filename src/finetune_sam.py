...
def build_model(conf):
    checkpoint_path = conf['model']['checkpoint']
    config_path = conf['model'].get('cfg', 'sam2_hiera_s.yaml')

    model = build_sam2(config_path, checkpoint_path)

    # Optionally freeze encoder for fine-tuning decoder only
    if conf['model'].get('freeze_encoder', True):
        ...
        for idx, batch in enumerate(test_loader):
            ...
            if not return_metrics_only:
                ...
                img_path = os.path.join(
                    conf['dataset']['root'],
                    test_loader.dataset.df.iloc[int(image_idx.item())]['enhanced_img_path']
                )
